...
print(thresh_df[thresh_df['threshold'].isin([0.45, 0.50, 0.55, 0.60, 0.65, 0.70, 0.75])
...
avg_diff = monthly_df['차이'].mean()
win_rate = (monthly_df['차이'] > 0).mean()
print(f"\n평균 차이    : {avg_diff:+.4f}")
print(f"모델우위 월수: {int((monthly_df['차이']>0).sum())} / {len(monthly_df)}개월 ({win_rate:.1%})")
...
print(f"Prob UP         : {tomorrow_proba[1]:.4f}")
print(f"Prob DOWN       : {tomorrow_proba[0]:.4f}")
